...
const GatewayConfig = () => {
  ...
  const handleOptionClick = (option) => {
    setSelectedOption((prevSelectedOption) =>
      prevSelectedOption.map((item, index) =>
        index === option ? { ...item, check: true } : { ...item, check: false },
      ),
    );
  };
          ...
          <div className="">
            ...
            <div className="">
              <h2 className="text-[1.8rem] mb-[.5rem] text-[#000000] font-[600]">
                Do you have an account with paystack?
              </h2>
                ...
                {selectedOption.map((option, index) => (
                  <button
                    ...
                    className={`
                                            ...
                                            ${option.check ? "bg-[#FBF0FF] sm:bg-[#FBF0FF]" : "bg-white sm:bg-white"}
            ...
            {selectedOption[0].check && (
              <div className="">
                <p className="text-[1.6rem] decoration-2 text-[#535862] mt-[3rem]">
                  Click the link below to sign up on paystack
                </p>
                <Link
                  to={"/"}
                  className="text-[1.6rem] underline text-[#3504FA] mt-[.9rem] block"
                >
                  https://paystack.com/signup
                </Link>
              </div>
            )}
            ...
            {selectedOption[1].check && (
              <div className="mt-[3rem]">
                ...
                <div className="mt-[4rem]">
                  ...
                  <div className="mt-[2.5rem] sm:ml-[1rem] ml-[4rem] flex flex-col gap-[2rem]">
                    <div className="text-[1.3rem]">
                      <Label
                        className="mb-[.5rem] block"
                ...
                <div className="mt-[3rem] sm:w-full sm:flex-col gap-[2rem] flex items-center">
                  <Button className="w-fit px-[3.8rem] py-[2rem] sm:w-full rounded-[.7rem] bg-[#FF9100] text-[1.6rem] font-[600]">
                    Save settings
                  </Button>

                  <Button
                    variant={"outline"}
                    className="w-fit sm:w-full border-[#D5D7DA] px-[2rem] py-[2rem] rounded-[.7rem] text-[1.6rem] font-[600] shadow-[0px_1px_2px_0px_#0A0D120D]"
                  >
                    Skip configuration settings
                  </Button>
                </div>
              </div>
            )}
